...
if (! $rides->driverAccept($rideId, $ctx['riderUserId'])) {
    http_response_code(409);
    echo json_encode(['error' => 'cannot_accept'], JSON_THROW_ON_ERROR);
    exit;
}
try {
    (new DriverAvailabilityRepository($pdo))->setStatus($ctx['riderUserId'], 'busy');
} catch (Throwable) {
    /* ignore */
}
