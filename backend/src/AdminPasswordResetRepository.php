...
final class AdminPasswordResetRepository
{
    public function __construct(private PDO $pdo) {}

    /**
     * Creates a new reset token; invalidates other pending tokens for this admin.
     *
     * @return non-empty-string raw token (only shown once — put in email)
     */
    public function createTokenForAdmin(int $adminId): string
    {
        $this->pdo->prepare(
            'DELETE FROM admin_password_resets WHERE admin_id = ? AND used_at IS NULL',
        )->execute([$adminId]);
        $raw = bin2hex(random_bytes(32));
        $hash = hash('sha256', $raw, false);
        $expires = date('Y-m-d H:i:s', time() + 3600);
        $this->pdo->prepare(
            'INSERT INTO admin_password_resets (admin_id, token_hash, expires_at) VALUES (?, ?, ?)',
    ...
    public function findValidRowByRawToken(string $rawToken): ?array
    {
        ...
        return $row === false ? null : ['id' => (int) $row['id'], 'admin_id' => (int) $row['admin_id']];
    }
